...
class database_parser(object):
    ...
    def add_urls(self, db_url):
        page = html.fromstring(urllib.urlopen(db_url).read()) #Fetches html from website
        for link in page.xpath("//a"): #To get text, it's link.text
            if '/' in link.get("href") and '/' in link.text:
                self.add_urls(db_url + link.get("href")) #LOOK! Recursion IN THE WILD! This means that the link is to a subdirectory, so runs the alg on this subdirectory
            elif '.mp3' in link.text: #Means the link is to an mp3 file, so adds it to the mp3 list
                mp3_name = link.text.replace('-', ' ').lower().strip()
